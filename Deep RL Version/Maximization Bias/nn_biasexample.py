...
class BiasLarge():
    def __init__(self, n_iter=1000, n_episodes=400, lr = 0.05):
        self.n_iter = n_iter
    ...
    def obtain_GoLeft(self):
        ...
        if (self.policy == 'D-Q' or self.policy == 'SORDQ') & (self.average == True):
            curQ = (curQ + QvalB) / 2
        if curQ[1] > curQ[0]:
            return 1
        if curQ[1] == curQ[0]:
    ...
    def update_q(self, QvalA, QvalB, state_old, action, reward, state_new, lr, done):
       ...
       #     print(QvalBNext)
        ...
        if self.policy == 'SORQ': #SOR Q-learning
            QvalA_old = self.Qa.predict(state_old)
            w = 1.3
            QvalA[action] = w * (reward + self.gamma * np.max(QvalANext)) + (1 - w) * np.max(QvalA_old)
            self.Qa.update(state_old,QvalA, lr)
        ...
        if self.policy == 'SORDQ': #SOR Double Q-learning

            if np.random.randint(2) < 1: #update Qa
                QvalA_old = self.Qa.predict(state_old)
                QvalB_old = self.Qb.predict(state_old)
                w = 1.3
                QvalA[action] = w * (reward + self.gamma * QvalBNext[np.argmax(QvalANext)]) + (1 - w) * QvalB_old[np.argmax(QvalA_old)]
                self.Qa.update(state_old,QvalA, lr)
            else:
                QvalA_old = self.Qa.predict(state_old)
                QvalB_old = self.Qb.predict(state_old)
                w = 1.3
                QvalB[action] = w * (reward + self.gamma * QvalANext[np.argmax(QvalBNext)]) + (1 - w) * QvalA_old[np.argmax(QvalB_old)]
                self.Qb.update(state_old,QvalB, lr)
